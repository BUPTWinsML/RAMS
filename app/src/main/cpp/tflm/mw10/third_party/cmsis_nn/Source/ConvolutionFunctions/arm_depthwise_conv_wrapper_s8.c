...
{
  static int cnt_ = 0;
  int in_ = input_dims->c * input_dims->h * input_dims->n * input_dims->w;
  int in_2 = input_dims->c * 3 * input_dims->n * input_dims->w;
  int out_ = output_dims->c * output_dims->h * output_dims->n * output_dims->w;
  int out_2 = output_dims->c * 1 * output_dims->n * output_dims->w;
  printf("[%d]: in_size = %d, in_size2 = %d, out_size = %d, out_size2 = %d\n", cnt_, in_, in_2, out_, out_2);
  cnt_++;
}
    ...
    }
{
  static int cnt_ = 0;
  if (cnt_ == 0) {
    for (int i = 0 * 36 * 16; i < 36 * 36 * 16; i++) {
    //   printf("%hhd, ", output[i]);
    }
    for (int i = 0; i < output_dims->c; i++) {
      // printf("%d, ", quant_params->multiplier[i]);
    }
    for (int i = 0; i < output_dims->c; i++) {
      // printf("%d, ", quant_params->shift[i]);
    }
  }
  cnt_ ++;
}
